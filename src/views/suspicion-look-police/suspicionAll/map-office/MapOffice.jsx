...
export default class Map extends Component {
  ...
  addHeatMap (e) {
    console.log(e, 'pp')
    let pointList = []
    ajax.getSearchPoliceStationCases({ policeKey: e.id }, response => {
    ...
    ajax.getStatisticsSubbureauCaseDistribution({}, response => {
      if (response.code === 101) {
        ...
        for (let i = 0; i < response.data.length; i++) {
          ...
          this.map.addCircle(response.data[i].centerLng, response.data[i].centerLat, 1000, {
            id: response.data[i].policeKey,
            outLineWidth: 2,
            outLineColor: [255, 0, 0, 0.7],
            color: [255, 153, 0, 0.6]
  ...
  render () {
    return (
      <div style={{ background: '#122346', height: '100%', paddingTop: '1vh' }}>
        <h3>盘龙分局案件分布区域</h3>
        <div ref='map' id='map' />
        <Modal title='' style={{width: '80%'}} visible={this.state.modalVisible} onCancel={this.handleCancel} afterClose={this.handleCancel} footer={null}>
          <ModalMap width='1000' />
        </Modal>
      </div>
    )
